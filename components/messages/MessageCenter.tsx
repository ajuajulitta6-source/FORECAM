...
const MessageCenter: React.FC = () => {
  ...
  return (
    <div className="flex h-[calc(100vh-8rem)] gap-6">
       ...
       <div className="w-full md:w-1/3 flex flex-col bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
           ...
           <div className="flex-1 overflow-y-auto">
               ...
               {filteredMessages.map(msg => (
                   <div 
                     key={msg.id} 
                     className={`p-4 border-b border-slate-50 cursor-pointer transition-colors hover:bg-slate-50 ${!msg.isRead && activeTab === 'INBOX' ? 'bg-blue-50/30' : ''}`}
       ...
       {/* Main View (Compose or Empty State for now, could be detail view) */}
       <div className="hidden md:flex flex-1 bg-white rounded-xl border border-slate-200 shadow-sm items-center justify-center p-8 text-slate-400 bg-slate-50/30">
